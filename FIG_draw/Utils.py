...
#     dic_l5: dict = field(init=False, repr=True)                     #qaoa
...
#         match_l5 = MATCH_PARA.findall(l5)                                   #qaoa
#         dist_l5 = {key: float(value) for key, value in match_l5}            #qaoa
#         object.__setattr__(self, "dic_l5", dist_l5)                 #qaoa
...
#     def cal_metric(self):
#         # print(self.dic_l2['cross_pair'])
#         if self.dic_l2.get('cross_pair',None) is not None: ## change!!
#
...
#             if instance.dic_l2.get('time_schedule', None) is not None: # change !!!
#                 metric_sums[file_p_key]['metric_t_sum'] += instance.dic_l2['time_schedule']
#             if instance.dic_l4.get('avg_wait_time', None) is not None: # change !!!
...
class file_data:
    ...
    def cal_metric(self):
        if self.dic_l2.get('cross_pair', None) is not None:  # change!!
            overhead = self.dic_l2['cross_pair'] + self.dic_l2['in_pair'] / 3 + self.dic_l2['post_in_pair'] * math.pow(0.7,
                                                                                                                   int(
                                                                                                                       self.file_p[
                                                                                                                           'split_mul']) - 1) / 3
            object.__setattr__(self, 'epr', overhead)
...
def calculate_cross_sublist_averages(big_list):
    metric_sums = defaultdict(
        lambda: {'metric_t_sum': 0.0, 'metric_w_sum': 0.0, 'epr_sum': 0.0, 'count': 0, "retry_num": 0, 'retry_time': 0,
                 'avg_retry_overhead': 0, 'cross': 0, 'in': 0, 'distill': 0})
    for sublist in big_list:
        for instance in sublist:
            filtered_file_p = {k: v for k, v in instance.file_p.items() if k != 'seed'}
            # if 'baseline'
            ...
            if instance.dic_l4.get('avg_wait_time', None) is not None: # change!!
                metric_sums[file_p_key]['metric_w_sum'] = instance.dic_l4['avg_wait_time']
            # metric_sums[file_p_key]['epr_sum'] += instance.epr change !!
            metric_sums[file_p_key]['count'] += 1
            ...
            if instance.dic_l2.get('cross_pair', None) is not None: # change!!
                metric_sums[file_p_key]['cross'] = instance.dic_l2['cross_pair']
            if instance.dic_l2.get('in_pair', None) is not None: # change!!
                metric_sums[file_p_key]['in'] = instance.dic_l2['in_pair']
            if instance.dic_l2.get('post_in_pair', None) is not None: # change!!
                metric_sums[file_p_key]['distill'] = instance.dic_l2['post_in_pair']
    averages = []
    for file_p_key, values in metric_sums.items():
        avg_metric_t = values['metric_t_sum'] / values['count']
        ...
        # print( values['count'])
        averages.append(
            {'file_p': dict(file_p_key), 'avg_metric_t': avg_metric_t, 'avg_metric_w': avg_metric_w, 'eproverhead': epr,
             'retry_num': retry_num, 'retry_time': retry_time, 'avg_retry_overhead': avg_retry_overhead, 'cross': cross,
             'in_pair': in_pair, 'distill': distill})
    return averages
...
def compute_dictionaries(data_path, eval, benchmark):
    ...
    for index, b in enumerate(benchmark):
        ...
        for aa in result_base[b]:
            # print('b', b)
            # print('avg_metric_t', aa['avg_metric_t'])
            # print('avg_metric_w', aa['avg_metric_w'])
            # print('eproverhead', aa['eproverhead'])
            diction_baseline[0][index].append(aa['avg_metric_t'])
            diction_baseline[1][index].append(aa['avg_metric_w'])
            diction_baseline[2][index].append(aa['eproverhead'])
